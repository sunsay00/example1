(module gqlqueries (gql-proxy-query-emit gql-query-emit gql-mutation-emit gqlqueries-generate-fixtures)
  ...
  (define (get-method-emit model method)
    (let* ((name (list (model->name model) (method->name method)))
      ...
      (list 
        "\nexport const " name " = async (resolver: Resolver, $ctx: IUserContext, " (intersperse (map param-emit params) ", ") "): Promise<" (paginated-type return-type-emit ret-type) "> => {"
        "\n  const query = `" (gql-query-emit model method) "`;"
        "\n  const json = await resolver.resolve('1', {}, query, {"
  ...
  (define (set-method-emit model method)
    (let* ((name (list (model->name model) (method->name method)))
           ...
           (ret-type (method->return-type method)))
      (list 
       "\nexport const " name " = async (resolver: Resolver, $ctx: IUserContext, " (intersperse (map param-emit params) ", ") "): Promise<" (cursorize return-type-emit ret-type) "> => {"
    ...
    (let* ((z 1)
           ...
           (expr
             (list
               ...
               "\n"
               "\nexport const createDeck = async (resolver: Resolver, $ctx: IUserContext, factoryId: string, title?: string, description?: string): Promise<Deck> => {"
               ...
               "\n      }"
               "\n    }`;"
               "\n  const createJson = await resolver.resolve('1', {}, createQuery, {"
               "\n    factoryId,"
               ...
               "\n};"
               "\n"
               "\nexport const removeDeck = async (resolver: Resolver, $ctx: IUserContext, deck: Deck): Promise<Deck> => {"
               "\n  const removeQuery = `mutation ($id: String!) {"
               "\n      decksRemove (id: $id) {"
               "\n        sub id user { name } title description"
               "\n      }"
               "\n    }`;"
               "\n  const removeJson = await resolver.resolve('1', {}, removeQuery, { id: deck.id }, $ctx);"
               "\n  if (removeJson.data == undefined) throw new Error('failed to remove deck');"
               "\n  return removeJson.data.decksRemove;"
               "\n};"
               "\n"
               "\nexport const findDeckById = async (resolver: Resolver, $ctx: IUserContext, deckId: string): Promise<Deck | undefined> => {"
               "\n  const deckQuery = `query ($id: String!) {"
               "\n      decksFindById (id: $id) {"
               "\n        sub id user { name avatarUri }"
               "\n      }"
               "\n    }`;"
               "\n  const result = await resolver.resolve('1', {}, deckQuery, { id: deckId }, $ctx);"
               "\n  if (result == undefined || result.data == undefined) return undefined;"
               "\n  return result.data.decksFindById as Deck;"
               "\n};"
               "\n"
               "\nexport const backendCreateUser = async (store: IStore, $ctx: IUserContext, name: string, avatarUri: string, locale: string, region: string): Promise<User> => {"
               "\n  return store.usersCreate($ctx, $ctx.sub, { name: name, avatarUri: avatarUri }, locale, region);"
               "\n};"
               "\n"
               "\nexport const backendRemoveUser = async (store: IStore, $ctx: IUserContext, id: string): Promise<User> => {"
               "\n  const result = await store.usersRemove($ctx, id);"
               "\n  if (result == undefined) throw new Error('user should not be undefined');"
               "\n  return result;"
               "\n}"
               "\n"
               "\nexport const backendFindUserById = async (store: IStore, $ctx: IUserContext, sub: string, id: string): Promise<User | undefined> => {"
