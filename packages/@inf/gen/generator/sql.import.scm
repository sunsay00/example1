      ...
      (match expr
             ...
             (`(quote ,x)
               (let ((type (param-name->method-type method x)))
                 (match type
                        (Point (list "ST_SetSRID(ST_MakePoint(${S(" x ".lon)}, ${S(" x ".lat)}), 4326)"))
                        (x x))))
             (x x)))
    (map recur exprs))
      ...
      (match v
             ...
             (else
               (let ((type (param-name->method-type method v)))
                 (match type
                        ('(Optional Point) (list "ST_SetSRID(ST_MakePoint(${!" v "?'" def ", " def "':`${S(" v ".lon)}, ${S(" v ".lat)}`}), 4326)"))
                        ('Point (list "ST_SetSRID(ST_MakePoint(${S(" v ".lon)}, ${S(" v ".lat)}), 4326)"))
                        ('(Optional Int)
                         (if count?
                           (list "${" v "==undefined?(" def "+1):S(Math.min(" def "+1, " v "+1))}")
                           (list "${" v "==undefined?" def ":S(" v ")}")))
                        ('Int
                         (if count?
                           (list "${" v "==undefined?(" def "+1):S(Math.min(" def "+1, " v "+1))}")
                           (list "${" v "==undefined?" def ":S(" v ")}")))
                        ('(Optional Boolean) (list "${S(" v "==undefined?false:" v ")}"))
                        ('(Optional String) (list "${!" v "&&" v "!=''?" def ":S(" v ")}"))
                        ('(Optional DateTime) (list "${!" v "?" def ":S(" v ")}"))
                        ('(Optional UserFields) (list "${!" v "?" def ":S(" v ")}"))
                        ('Boolean (list "${S(" v ")}"))
                        ('String (list "${S(" v ")}"))
                        ('DateTime (list "${S(" v ")}"))
                        (`(Array ,t) (list "${S(" v ")}"))
                        ('CommentItem (list "${S(" v ")}"))
                        ('Appointment (list "${S(" v ")}"))
                        ('UserFields (list "${S(" v ")}"))
                        (else (error (smoosh "(sql.import) unhandled push type " (->string type) " for " v))))))))
    (define (push v) (push/default v "null" #f))
    (define (push-list l) (list "${" l ".map(n => `${S(n)}`).join(', ')}"))
