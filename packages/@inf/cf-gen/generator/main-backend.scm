...
(define (run-sub entire-api ledger output-dir)
      ...
      (lambda (schema i) 
        ...
        (define (generate-migration timestamp name)
          (let ((z (number->string (inexact->exact (floor (version-name->number name))))))
            (save (migrations-generate-up entire-api api delta-api i) (get-path (string-append "infra/postgres/migrations/sqls/" timestamp "-v" z "-up.sql")))
            (save (migrations-generate-down api delta-api i) (get-path (string-append "infra/postgres/migrations/sqls/" timestamp "-v" z "-down.sql")))
            (save (migrations-generate-js timestamp name) (get-path (string-append "infra/postgres/migrations/" timestamp "-v" z ".js")))))
...
(define (run ledger output-dir)
  (let ((schemas (generate-schemas #t ledger)))
    (let ((i 0) (schema (if (= (length schemas) 1) (car schemas) (error "invalid schema"))))
      ...
      (save (typedefs-generate api) (get-path (string-append "api/resolver/" (symbol->string v) "/typedefs.ts")))
      (save (resolvers-generate api) (get-path (string-append "api/resolver/" (symbol->string v) "/resolvers.ts")))
      (save (mappers-generate prev-api delta-api vnum) (get-path (string-append "api/mapper/mapper_" (symbol->string v) ".ts")))
      (save (mappers-generate-index api vnum) (get-path "api/mapper/index.ts"))

      (define last? (= (+ i 1) (length schemas)))
      (if last?
        (begin 
          (save (service-interfaces-generate api) (get-path "types/serviceinterfaces.ts"))

          (map (lambda (name)
                 (save (services-generate name api) (get-path (string-append "api/services/basic/" (symbol->string (lower name)) "service.ts"))))
               (map model->name (api->models api)))

          (save (mockstores-generate api) (get-path "__tests__/mocks/mockstore.ts"))
          ;(save (playbackstores-generate api) "../back/api/src/api/__tests__/mocks/playbackstore.ts")
          (save (store-interfaces-generate api) (get-path "types/storeinterfaces.ts"))
          (save (cachestore-generate api) (get-path "api/stores/cachestore.ts"))
          (save (rdsstore-generate api) (get-path "api/stores/rdsstore.ts"))
          (save (types-generate api) (get-path "types/models.ts"))

          (let ((output (gqlqueries-generate-fixtures api)))
            (save output (get-path "__tests__/fixtures/index.ts")))

          (let ((output (connectors-generate api)))
            (save output (get-path "api/connectors.ts")))
...
(if (not (= (length args) 2))
  ...
  (begin
    ...
    (run-sub entire-api ledger (cadr args))))
